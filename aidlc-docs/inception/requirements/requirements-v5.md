...
#### Death 完了→エンティティ削除シーケンス（B-NG-1 / Q-NG-6 / B-HIGH-01対応）
...
3. **AnimationSystem**: `mixer.addEventListener('finished', handler)` で Death clip 完了を検知し、entity に `DeathCompleteFlag { linger: 0.3 }` を付与（linger=秒、最終ポーズ保持時間）
4. **CleanupSystem**（毎フレーム）:
   - `DeathCompleteFlag` を持つ entity について `linger -= dt`
   - `linger > 0` の間は mesh をそのまま残し最終ポーズを表示（mixer.update継続、`clampWhenFinished` により静止）
   - `linger <= 0` に到達した entity のみ以下を順に実行:
     - (a) XPドロップ（既存 Iter4 実装どおり）
...
- linger 時間は **0.3秒** を暫定値（Construction Day 1 調整）、死体滞留と演出なめらかさのトレードオフ
- Death 中にダメージを受けても HitReact には遷移しない（優先度 Death > HitReact）
- `DeathCompleteFlag` の型定義:
  ```ts
  class DeathCompleteFlag {
    linger: number = 0.3;  // 秒、最終ポーズ保持時間
  }
  ```
...
#### 反転ハルの bind 手順（B-NG-5 / F-HIGH-02対応、SkeletonUtils.clone は outline 用に使わない）

1. 本体 root（`SkeletonUtils.clone(gltf.scene)` で取得済み）を scene に add
2. 本体 root を `traverse` し、各 SkinnedMesh について以下を実施:
   a. `geometry.clone()` で geometry を複製
   b. BackSide 用 ShaderMaterial を生成（`skinning` flag なし、vertex shader 内で `#include <skinning_pars_vertex>` / `<skinbase_vertex>` / `<skinning_vertex>` / `<skinnormal_vertex>` を使う）
   c. `new THREE.SkinnedMesh(clonedGeo, outlineMat)` で outline mesh を生成
   d. `outlineMesh.bind(bodyMesh.skeleton, bodyMesh.bindMatrix)` で**本体と同じ skeleton を共有**
   e. `bodyMesh.parent.add(outlineMesh)` で本体の兄弟として配置（同じ bone 階層を参照）
3. 毎フレーム `mixer.update(dt)` は本体 root に対して1回呼べば、skeleton.boneMatrices が共有されるため outline mesh にも反映される（outline 用 mixer は不要）
4. **SkeletonUtils.clone は outline 用には使わない**（独自 skeleton が生成され本体と同期しないため）
...
- payload 上限（S-NG-6 / I-NG-1 / A-NG-5対応）: モデル各 **3MB 上限**（Character 実測 Soldier=2.34MB / Enemy=2.27MB / Hazmat=2.33MB を考慮し 2MB → 3MB へ引き上げ）、超過検知時はエラー（MVP）。種別別上限は採用せず、シンプルさ優先で全GLTFに一律適用

#### エラーメッセージ3層分界（S-NG-3対応）

| 層 | 表示内容 | 例 |
|---|---|---|
| UI層（LoaderScreen / エラーダイアログ） | **固定エラーID のみ**（汎用文言マッピング） | `FATAL_ASSET_LOAD_FAILURE`, `アセットの読み込みに失敗しました` |
| console 層（開発者向け） | URL / stack / bone名 / サイズ等の詳細 | `[EntityFactory] bone 'Hand.R' not found in Character_Soldier at ...` |
| 外部ログ層（将来 Sentry 等） | redact 必須 | 現Iter5は対象外、Iter6+で scrub rules 整備 |

**throw 規約**: `Error.message` = 固定ID、`Error.cause` = 詳細object。UI層は `error.message`（固定ID）のみマッピング表示、詳細は `console.error(err)` のみ。
...
### NFR-08: CSP整合（S-NG-1 / I-NG-4対応）

- Iter3 NFR-06 のXSS/CSP方針を継承（`innerHTML` 禁止、`textContent` / DOM API 構築）
- **Iter5 で実際に必要な CSP**（最小権限、blob: / worker-src 不要）:
  - `default-src 'self'`
  - `script-src 'self'`（`'unsafe-eval'` 不要、ShaderMaterial は GLSL を GPU に渡すのみで JS eval は使わない）
  - `img-src 'self' data:`（glTF インライン texture の dataURI 利用、`blob:` は未使用）
  - `connect-src 'self'`（`fetch(url)` による GLTF 取得）
- LoaderScreen の DOM 構築は `textContent` と DOM API のみ

**将来 CSP 追加トリガ**（Iter5 対象外）:
- DRACO / meshopt 圧縮採用時: `worker-src 'self' blob:`, `script-src 'self' blob:` を追加
- KTX2 / Basis texture 採用時: 同上（Web Worker + blob URL）
...
- デプロイ検証（I-NG-6 / O-NG-8対応、Iter5 スコープ縮退）:
  - **Iter5 時点**: CI でビルド artifact 内のファイル存在確認のみ（デプロイ先未確定のため）
    ```bash
    test -f dist/models/toon-shooter/v1/characters/Character_Soldier.gltf
    # 他11ファイルも同様
    ```
  - **Iter6 以降**: デプロイ先確定後、`curl -I` で 200 応答検証を追加
