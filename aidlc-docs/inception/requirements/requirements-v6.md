...
| **ゲート** | **プレイヤーの通過で発動**（撃破不要、撃てない、仲間単独通過はスルー） | 発動時に仲間追加 or 全仲間にバフ（攻撃UP / 移動速UP / HEAL 等） | **縦型ゲート（Last War 風、プロシージャル）**: 台座 + テーパー柱 + 柱頂フィニアル + エネルギーカーテン（追加合成 Plane）+ 下端アクセントライン。上部横棒なし。**効果量を大きく数値表示** |
...
- **数値ラベル表示**: **効果量 + 種別ラベル**（`+5 ALLY` / `+30% ATK` / `+20% SPD` / `+40 HP`）をゲートにオーバーレイ表示（HTMLOverlayManager 経由、既存 HUD サイズ系列、最小 20px）。初見ユーザーが効果種別を判別できるよう i18n ラベルを数値の横に併記する。
- **見た目**: 縦型ゲート（Last War 風、プロシージャル）— 台座 + テーパー柱 + 柱頂フィニアル + エネルギーカーテン（追加合成 Plane）+ 下端アクセントライン。上部横棒なし、通過ポイントが発光で強調される。バフ種別で色分け（例: ALLY_ADD=青 / ATTACK_UP=赤 / SPEED_UP=黄 / HEAL=緑）
