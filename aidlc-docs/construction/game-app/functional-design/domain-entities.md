...
| 属性 | 型 | 説明 | 初期値 |
|------|-----|------|--------|
| position | Position | 画面上の座標 | (360, 1200) 画面下部中央 |
| hp | number | 現在HP | 100 |
...
| colliderRadius | number | 衝突判定半径(px) | 16 |
...
| colliderRadius | number | 衝突判定半径(px)。固定値 4 |
| ownerId | EntityId | 発射元エンティティ |

### E-04: XPDrop（経験値アイテム）
敵撃破時にドロップする経験値宝石。生成後その場に静止する（移動しない）。
...
| spriteHalfWidth | number | 描画半幅(px)。画面境界制限に使用。初期値 12 |
...
|--------|-----|------------|-------------|-----------|---------|
| NORMAL | 20 | 80 | 10 | 10 | 12 |
| FAST | 10 | 160 | 8 | 15 | 12 |
| TANK | 60 | 40 | 15 | 25 | 20 |
| BOSS | 500+ | 30 | 30 | 200 | 40 |
...
| プレイヤー初期位置 | (360, 1200) | 画面下部中央 |
| 防衛ライン位置 | Y = 1248 | 画面下端から32px上 |
| 敵スポーン領域 | Y = -50〜-10, X = 0〜720 | 画面上部外 |
| XP回収半径 | 48px | プレイヤー中心からの距離 |
