...
```
2D論理座標 → 3Dワールド座標:
  worldX = gameX * SCALE                    (0-720px → 0-7.2units)
...
### フラスタム計算による検証
```
ゲーム領域: 7.2 x 12.8 worldunits (720x1280px)

カメラ設定（調整後）:
  FOV: 60度（45→60に拡大）
  position: (3.6, 12, 12)  // 高さ・距離を増加
  lookAt: (3.6, 0, -6.4)   // ゲーム領域中央を注視

検証:
  カメラ→lookAt距離 = sqrt(0 + 144 + 338.56) ≈ 22.0 units
  垂直FOV=60度: 可視高さ = 2 * 22.0 * tan(30°) ≈ 25.4 units > 12.8 ✓
  水平FOV(9:16): 可視幅 = 25.4 * (9/16) ≈ 14.3 units > 7.2 ✓
  
  → ゲーム領域全体が余裕をもってフラスタム内に収まる
```
